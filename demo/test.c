...
int test1()
{
	...
	int i = 0;
	string = "A=[7 4 5;5 3 6 ;5 6 0.7;7.3 4 1]";
	...
	while ((rc = pcre2_match(re, subject, subject_length, start_offset, 0,
		...
		printf("out vector[0] %d ", (int)ovector[0]);
		printf("out vector[1] %d :", (int)ovector[1]);
	...
	re = pcre2_compile((PCRE2_SPTR)pattern_colon, PCRE2_ZERO_TERMINATED, 0,
			   &errorcode, &erroroffset, NULL);
	if (re == NULL) {
		PCRE2_UCHAR buffer[256];
		pcre2_get_error_message(errorcode, buffer, sizeof(buffer));
	...
	subject_length = (PCRE2_SIZE)strlen(string);
	start_offset = 0;
	...
	puts("--------------");
	printf("found %d numbers, found %d colons\n", number_num, colon_num);
}
...
defined to be size_t. */
	...
	pattern = (PCRE2_SPTR) "(?'NUMBER'(-?\\d+(,\\d+)*(\\.\\d+)?))";
	subject = (PCRE2_SPTR) "A=[7 4 5;5 3 6 ;5 6 0.7;7.3 4 1]";
...
sequence. */
  ...
  offset that was passed to pcre2_match(). */
			...
			if (start_offset <= startchar) {
				...
				start_offset =
					startchar +
					1; /* Advance by one character. */
				if (utf8) /* If UTF-8, it may be more  */
				{ /*   than one code unit.     */
					for (; start_offset < subject_length;
					     start_offset++)
						if ((subject[start_offset] &
						     0xc0) != 0x80)
							break;
				}
			}
  ...
  UTF mode. */
		...
		if (rc == PCRE2_ERROR_NOMATCH) {
			...
			ovector[1] =
				start_offset + 1; /* Advance one code unit */
			if (crlf_is_newline && /* If CRLF is a newline & */
			    start_offset <
				    subject_length - 1 && /* we are at CRLF, */
			...
			{ /* advance a whole UTF-8 */
				while (ovector[1] <
				       subject_length) /* character. */
				{
					if ((subject[ovector[1]] & 0xc0) !=
					    0x80)
						break;
					ovector[1] += 1;
				}
